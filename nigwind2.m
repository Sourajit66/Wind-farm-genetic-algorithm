...
%layoutFile = 'Shell_Hackathon Dataset\try9.csv';
% Power curve
powerCurveFile = 'Shell_Hackathon Dataset\power_curve.csv';
% Wind Data 
windDataFile = 'Shell_Hackathon Dataset\Wind Data\wind_data_2013.csv';


w=array2table(w)

%writematrix(w,sprintf('try%d.csv',k))
    %end
%s1='Shell_Hackathon Dataset';
%Q='tryx1.csv'
%s=strcat(s1,s2)
%op=controlled_farm_evaluator(w);
%disp(op)
% Read the above data files
turb_coords  = table2array((w));    
...
%%
save('secondyr.mat','sas')
%sas=array2table(sas)
%disp(sas)
%sas=sas'
%writematrix(sas,"sas1.csv")
